...
def find_file(filename):
    if filename:
        ...
        fpath = cv2.samples.find_file(filename, False)
        if fpath:
            return fpath
        ...
        for path in ['OPENcv2_DNN_TEST_DATA_PATH', 'OPENcv2_TEST_DATA_PATH']:
            try:
        ...
        print('File ' + filename + ' not found! Please specify a path to '
              '/opencv2_extra/testdata in OPENcv2_DNN_TEST_DATA_PATH environment '
...
def draw_vectors(points, pose_pairs, body_parts, pair_colors, frame):
    for pair, color in zip(pose_pairs, pair_colors):
        ...
        if points[id_from] and points[id_to]:
            cv2.line(frame, tuple(points[id_to]), tuple(points[id_from]), color, 3)
            cv2.ellipse(frame, tuple(points[id_to]), (5, 5), 0, 0, 360, color, cv2.FILLED)